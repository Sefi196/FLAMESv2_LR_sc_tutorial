...
perform_trajectory_analysis <- function(seurat_obj, assay, prefix, cluster_name="harm_cluster", 
  ...
  deg_results <- graph_test(cds, neighbor_graph = 'principal_graph', cores = cores) # need to fix this bug of cds not filtering properly 
...
```{r traj_genes, echo=TRUE, fig.height=8, fig.width=8, message=FALSE, warning=FALSE}
perform_trajectory_analysis(seurat_obj = obj, "RNA", prefix = "Multi-sample_tutorial",
                            cluster_name="sctype_db",  root_cells = NULL, cores = 8, order_features_by_pseudotime = TRUE)

```

```{r traj_iso, echo=TRUE, fig.height=8, fig.width=8, message=FALSE, warning=FALSE}
