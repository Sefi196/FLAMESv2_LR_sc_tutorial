...
[^09_trajectory-1]: It’s important to note that analyzing features in relation to pseudotime can be computationally intensive Users can choose to plot the trajectory without the added computational load of ordering features by pseudotime, making the analysis more flexible. The parameter `number_of_features` can be used to specify the number of highly variable features to test.
    ...
    ```{r traj_fucntion, echo=TRUE, include=FALSE}
    ...
    perform_trajectory_analysis <- function(seurat_obj, assay, prefix, cluster_name="harm_cluster", 
      ...
      # Extract the top 100 variable gene names
      ...
      monocle_plot <- monocle3::plot_genes_in_pseudotime(cds = cds_top_genes, min_expr = 0.5, cell_size = 0.5,
                                         nrow = NULL, ncol = 3, panel_order = NULL,
...
```{r traj_genes, echo=TRUE, fig.height=10, fig.width=10, message=FALSE, warning=FALSE}
perform_trajectory_analysis(seurat_obj = obj, "RNA", prefix = "Multi-sample_tutorial",
                            cluster_name="sctype_db",  root_cells = NULL, cores = 8, order_features_by_pseudotime = TRUE, number_of_features = 100)

```

The same analysis can be perform at the isoform level. Set the `assay = "iso"`.
...
perform_trajectory_analysis(seurat_obj = obj, assay="iso", prefix = "iso_Multi-sample_tutorial",
                            cluster_name="sctype_db",  root_cells = NULL, cores = 8, order_features_by_pseudotime = FALSE, number_of_features = 500)
