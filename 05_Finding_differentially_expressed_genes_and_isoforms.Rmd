...
all_markers_gene_celltype %>%
    ...
    slice_head(n = 5) %>%
    ungroup() -> G_top5
DoHeatmap(seu_obj, features = G_top5$gene, assay = "RNA") + NoLegend() 
...
all_markers_iso_celltype %>%
    ...
    slice_head(n = 5) %>%
    ungroup() -> I_top5
DoHeatmap(seu_obj, features = I_top5$gene, assay = "iso") + NoLegend() 
```

Personally i find the same plots generated by `dittoHeatmap` much nicer. The function can also take a list of genes or isofroms of interest. Just remember to set the default assay accordingly.
